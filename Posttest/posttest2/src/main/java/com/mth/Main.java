...
public class Main {
    ...
    private void MenuKelolaSoftware() {
        ...
        if (dataSoftware.isEmpty()) {
            ...
            for (int i = 0; i < dataSoftware.size(); i++) {
                System.out.println(i + 1 + ". " + dataSoftware.get(i).GetNama());
            }
        ...
        if (indexDipilih >= 0 && indexDipilih < dataSoftware.size()) {
            System.out.println();
            System.out.println("Pilih tindakan untuk \"" + dataSoftware.get(indexDipilih).GetNama() + "\": ");
            ...
            _temp = input.nextLine();
            switch (_temp) {
                case "1" -> EditSoftware(indexDipilih);
                case "2" -> HapusSoftware(indexDipilih);
                default -> {
                    System.out.print("Pilihan tidak valid!");
                    input.nextLine();
                }
    ...
    private int GetSoftwareIndex(String nama) {
        for (int i = 0; i < dataSoftware.size(); i++) {
            if (nama.equals(dataSoftware.get(i).GetNama())) {
                return i;
            }
        }
        return -1;
    ...
    private void EditSoftware(int id) {
        ...
        System.out.println("———————————————————————————————————————————");
        System.out.println("Edit Proyek \"" + dataSoftware.get(id).GetNama() + "\"");
        System.out.println();

        System.out.print("Nama Proyek (\"" + dataSoftware.get(id).GetNama() + "\"): ");
        _temp = input.nextLine();
        _namaProyek = _temp.isBlank() ? dataSoftware.get(id).GetNama() : _temp;
        ...
        System.out.println("3. Lainnya");
        do {
            System.out.print("Masukkan pilihan tipe (\"" + dataSoftware.get(id).GetTipe() + "\"): ");
            _temp = input.nextLine();
            if (_temp.isBlank()) {
                _tipeProyek = dataSoftware.get(id).GetTipe();
                break;
            }
        ...
        System.out.println();
        do {
            System.out.print("Harga (Rp" + dataSoftware.get(id).GetHarga() + "): Rp");
            _tempInt = input.nextInt();
            input.nextLine();
            if (_tempInt < 0) {
                System.out.println("Harga tidak boleh kurang dari 0");
        ...
        System.out.println();
        System.out.println("(Deskripsi lama: \"" + dataSoftware.get(id).GetDeskripsi() + ")\"");
        System.out.print("Deskripsi Proyek: ");
        _temp = input.nextLine();
        _deskripsiProyek = _temp.isBlank() ? dataSoftware.get(id).GetDeskripsi() : _temp;

        dataSoftware.get(id).SetNama(_namaProyek);
        dataSoftware.get(id).SetTipe(_tipeProyek);
        dataSoftware.get(id).SetHarga(_hargaProyek);
        dataSoftware.get(id).SetDeskripsi(_deskripsiProyek);
    ...
    private void HapusSoftware(int id) {
        System.out.print("Apakah Anda yakin ingin menghapus \"" + dataSoftware.get(id).GetNama() + "\"? (y/n): ");
        do {
            _temp = input.nextLine();
            switch (_temp) {
                case "Y", "y" -> {
                    System.out.print("Berhasil menghapus \"" + dataSoftware.get(id).GetNama() + "\"!");
                    input.nextLine();
